# Go-Spec-Mock Configuration Options

Here's the configuration documentation for the `Go-Spec-Mock` project, detailing the configuration options, their sources, and precedence.

**Configuration Precedence (Highest to Lowest):**

1.  **Explicit CLI flags:** Values provided directly via command-line arguments.
2.  **Environment variables:** Values set as environment variables.
3.  **Configuration file values:** Values loaded from a YAML or JSON configuration file.
4.  **CLI flag default values:** Default values assigned to CLI flags if not explicitly set.
5.  **Default configuration values:** Hardcoded default values within the application.
...
| :---------------------------- | :------------ | :------- | :------------------- | :------------ | :---------- |
| `Config.SpecFile`             | `spec_file`   | `--spec-file` | `GO_SPEC_MOCK_SPEC_FILE` | `""` (empty string) | Path to the OpenAPI specification file. |
...
| Config Field (Go Struct Path) | YAML/JSON Key | CLI Flag | Environment Variable | Default Value | Description |
| :---------------------------- | :------------ | :------- | :------------------- | :------------ | :---------- |
| `Config.Server.Host`          | `server.host` | `--host` | `GO_SPEC_MOCK_HOST` | `localhost` | Host to run the mock server on. |
| `Config.Server.Port`          | `server.port` | `--port` | `GO_SPEC_MOCK_PORT` | `8080` | Port to run the mock server on. |
| `Config.Server.MetricsPort`   | `server.metrics_port` | `--metrics-port` | `GO_SPEC_MOCK_METRICS_PORT` | `9090` | Port to run the metrics server on. |
| `Config.Server.ReadTimeout`   | `server.read_timeout` | N/A | `GO_SPEC_MOCK_READ_TIMEOUT` | `15s` | Read timeout for the server. |
| `Config.Server.WriteTimeout`  | `server.write_timeout` | N/A | `GO_SPEC_MOCK_WRITE_TIMEOUT` | `15s` | Write timeout for the server. |
| `Config.Server.IdleTimeout`   | `server.idle_timeout` | N/A | `GO_SPEC_MOCK_IDLE_TIMEOUT` | `60s` | Idle timeout for the server. |
| `Config.Server.MaxRequestSize` | `server.max_request_size` | N/A | `GO_SPEC_MOCK_MAX_REQUEST_SIZE` | `10485760` (10MB) | Maximum request body size. |
| `Config.Server.ShutdownTimeout` | `server.shutdown_timeout` | N/A | `GO_SPEC_MOCK_SHUTDOWN_TIMEOUT` | `30s` | Shutdown timeout for the server. |
...
| Config Field (Go Struct Path) | YAML/JSON Key | CLI Flag | Environment Variable | Default Value | Description |
| :---------------------------- | :------------ | :------- | :------------------- | :------------ | :---------- |
| `Config.Security.Auth.Enabled` | `security.auth.enabled` | `--auth-enabled` | N/A | `false` | Enable API key authentication. |
| `Config.Security.Auth.HeaderName` | `security.auth.header_name` | N/A | N/A | `X-API-Key` | HTTP header name for API key. |
| `Config.Security.Auth.QueryParamName` | `security.auth.query_param_name` | N/A | N/A | `api_key` | Query parameter name for API key. |
| `Config.Security.Auth.Keys`   | `security.auth.keys` | N/A | N/A | `[]` (empty list) | List of API keys. |
