...
func TestTracer_StartSpan(t *testing.T) {
	...
	ctx, span := tracer.StartSpan(ctx, spanName, attrs...)
	if span == nil {
...
func TestTracer_ContextPropagation(t *testing.T) {
	...
	ctx := context.Background()
	ctx, span := tracer.StartSpan(ctx, "initial-span", attribute.String("initial", "true"))
	
	// Propagate context to new span
	ctx, newSpan := tracer.StartSpan(ctx, "propagated-span", attribute.String("propagated", "true"))
	
	// Verify spans have different contexts but are related (noop tracer may not have valid context)
	initialSpanCtx := trace.SpanFromContext(ctx).SpanContext()
...
func TestTracer_ConcurrentSpans(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		go func(id int) {
			ctx := context.Background()
			ctx, span := tracer.StartSpan(ctx, "concurrent-span", attribute.Int("id", id))
			span.End()
			done <- true
		}(i)
	}
...
func TestTracer_NilContext(t *testing.T) {
	...
	ctx := context.Background()
	ctx, span := tracer.StartSpan(ctx, "nil-context-test")
